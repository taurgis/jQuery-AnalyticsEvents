...
EasyAnalyticsEvents =  {
	...
	init: function(options) {
		...
		$("." + options.clickClass).on('click', { options: options }, function(event) {
			var _this = $(this);
	...
	track: function(eventArguments, options) {
		...
		if (_gaq) {
			 ...
			 ga('send', 'event', eventArguments.category, eventArguments.action, eventArguments.label, eventArguments.value, { 'nonInteraction' : args.nonInteraction });
		} else {
